...
SELECT concat(first_name, " ",  last_name) AS customer_name, SUM(amount) as total_revenue 
FROM payment LEFT JOIN customer ON payment.customer_id = customer.customer_id WHERE first_name LIKE ("S%") GROUP BY customer_name HAVING total_revenue > 150.00  ORDER BY total_revenue DESC LIMIT 10;